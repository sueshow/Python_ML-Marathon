...
* 梯度下降(Gradient Descent)：最常用的優化算法
  * 定義：機器學習算法當中，優化算法的功能，是通過改善訓練方式，來最小化(或最大化)損失函數
    * 除梯度下降(以ㄑ外，有：最小平方法、SVD分解，這些都有 close form solution
  * 概念
  ...
  * 確定到極值點？
    ...
    * momentum 是梯度下降法中一種常用的加速技術
      * Gradient Descent 的實現：SGD
        ...
        * 帶 momentum 項的 SGD 如下形式：
          $$ v =  ß * v − a * dx $$
          $$ x ← x + v $$
          * 說明：其中 $ß$ 為 momentum 係數，若上一次的 momentum(即$ß$)與這一次的負梯度方向是相同的，那這次下降的幅度就會加大，這樣做能夠達到加速收斂的過程 
    * avoid local minima
