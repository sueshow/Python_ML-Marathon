...
* 決策樹 Decision Tree
  ...
  * 從訓練資料中找出規則，讓每一次決策能使訊息增益 (Information Gain) 最大化，訊息增益越大代表切分後的兩群資料，群內相似程度越高
    * 吉尼係數(Gini-Index)：$Gini = 1-\sum_j p_j^2$
    * 熵(Entropy)：$Entropy = -\sum_j p_j log_2 P_j$
...
* 練習
  ...
  * 8：切換正規化選項與參數(正規化對於模型的影響性)
    ...
    * 實驗結果：可處理 overfitting 的問題、預防已知答案
    ...
    ![切換正規化選項與參數](https://github.com/sueshow/Python_ML-Marathon/blob/main/Picture/TF%20PlayGround_%E5%88%87%E6%8F%9B%E6%AD%A3%E8%A6%8F%E5%8C%96%E9%81%B8%E9%A0%85%E8%88%87%E5%8F%83%E6%95%B8.png)
  * 重點
...
* 深度學習網路歷史<br>
  ...
  * 單層感知器(Perceptron)
    * 感知器：最簡單形式的前饋式神經網路，沒有方向性，根據資料訓練預測的目標
    * 缺點：無法泛化，容易 overfitting
  * 多層感知器(Multilayer Perceptron)
    * 加深單層感知器，加了隱藏層(TLU)，解決 XOR 的問題
  * 深度學習網路(如DNN)
    * 優化後的 MLP，更多隱藏層
  * 差異
...
* 深度學習網路
  * 概念：從資料中尋找一組最適合解決某種特定問題的函數
    ...
    * Pick the best Function
      * 使用最佳化方法 Gradient Descent(梯度下降法)修正神經網路中的權重(W)，降低在訓練過程中產生的誤差
      * Gradient Descent 步驟
        * Loss Function 對網路中的各個 W 進行偏微分，讓各個 W 權重逐步逼近最適合的權重
        * 針對這 $y_1$~$y_M$ 函數中(模型預測的結果)，各個權重進行偏微分，算出 W 應該要修正的數值，並重複針對輸入下一批資料進行權重修正
        * 用 Loss function 針對各個 W 權重作偏微分，調整各個權重(W)的值，反覆進行，最後目標是 Y1~Y10 的預測機率能逼近損失數值(Loss)最小的狀態，此時整個模型以及裡面的權重組合為 Best Function
  * 優點
    * 發展多種架構應用於各領域中，如 CNN、RNN、GAN
    * 進行較深層的運算取出更多有用的資訊提升準確度
  * 缺點
    * 隱藏層增加更容易使得神經網路陷入區域最佳解，無法收斂
    * 通過越多隱藏層產生的梯度變化就會越來越小，直到看不太出來變化，導致產生梯度消失問題
    * 較深層的神經網路可能使模型陷入過擬合情形，需適當的使用正規化或 dropout 做處理
* 範例與作業(待上傳)
  * [範例D070]()
    * 了解 MLP 在神經網路上的應用
    * 資料集：手寫辨識的數據集
  * [作業D070]()
    * 了解 MLP 中神經網路層多寡的預測結果差異
    * 資料集：手寫辨識的數據集
